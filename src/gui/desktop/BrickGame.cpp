...
// Так как считывание сигналов идёт с интерфейса, и для скорости оставил
// обработку сигналов в view, поэтому убрал прослойку контроллера
...
void BrickGame::showGameWin(const GameInfo_t& game_info) {
  showMessage("You Win!", "Congratulations!\nYour score: " +
                              QString::number(game_info.score));
}
...
void BrickGame::drawPausedScreen(QPainter& painter) {
  QRect textRect(gameBoardRect.x(), gameBoardRect.y(), gameBoardRect.width(),
...
void BrickGame::drawGameField(QPainter& painter, const GameInfo_t& game_info) {
  ...
  for (int row = 0; row < ROWS_MAP; ++row) {
    for (int col = 0; col < COLS_MAP; ++col) {
      QRect cellRect(xOffset + col * cellSize, yOffset + row * cellSize,
                     cellSize, cellSize);
...
void BrickGame::drawNextTetramino(QPainter& painter,
  ...
  QRect secondRect = statsFrame->geometry();
  int miniCellSize = nextTetrominoRect.width() / 4;
